...
export const AddBook = async(req,res) => {
 ...
 try {
    ...
    const book_cover = req.file ? req.file.path : "";

    // We generally don’t take the image URL from req.body because the URL is not known at request time—it is generated by the backend after uploading the file to Cloudinary, so it naturally lives in req.file.path. Accepting image URLs directly from req.body is also unsafe, because any client could send a fake or malicious URL, bypassing file validation, size limits, and type checks. Using backend-handled uploads ensures the server owns the upload process, enforces validation, and guarantees that the stored URL actually points to a successfully uploaded image. In short: clients send files, servers generate and trust URLs, which is why req.file.path is the correct source, not req.body.
