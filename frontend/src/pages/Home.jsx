...
const Home = () => {
  ...
  const handleRentClick = (book) => {
    toast.success(
      `To rent "${book.book_title}", please visit our physical store location. Our staff will be happy to assist you!`,
      {
        duration: 5000,
        style: {
          maxWidth: '500px',
  ...
  return (
    ...
    >
      <nav className="bg-white/70 backdrop-blur-xl shadow-sm border-b border-white/40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-amber-600 font-semibold">Library</p>
              <h1 className="text-2xl font-bold text-slate-900">Book Collection</h1>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        {/* Search Bar */}
        <div className="bg-white/75 border border-white/60 backdrop-blur-2xl rounded-3xl shadow-[0_24px_80px_-45px_rgba(0,0,0,0.55)] p-6">
          <input
            type="text"
            placeholder="Search books by title, author, or language..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-400 focus:border-transparent outline-none shadow-sm bg-white/90 text-slate-900 placeholder:text-slate-400 backdrop-blur"
          />
        </div>
                  ...
                  <button
                    ...
                    className={`w-full py-3 rounded-xl font-semibold transition-all ${
                      book.book_quantity === 0
                        ...
                        : 'bg-gradient-to-r from-amber-500 to-rose-500 text-white hover:shadow-lg hover:-translate-y-0.5'
                    }`}
                  >
                    {book.book_quantity === 0 ? 'Out of Stock' : 'Rent Book'}
                  </button>
                </div>
              </motion.div>
            ))}
